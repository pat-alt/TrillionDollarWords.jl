...
sorted_dir_files = readdir(out_dir)[contains.(readdir(out_dir), ".csv")][files_sortperm]
...
# Post-process:
df = DataFrame(Arrow.Table(joinpath(merge_dir, "activations.arrow"))) 

# Check that all sentences are present:
df_all = load_all_data()
all_ids = unique(df_all.sentence_id) |> sort
q = df.sentence_id |> @unique()
